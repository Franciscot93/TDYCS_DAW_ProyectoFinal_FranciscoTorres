...
// private baseUrl = `http://localhost:${config.ports.datos}/`;
...
export class UsuarioController {
  ...
  async crearUsuario(req: Request, res: Response) {
    try {
      ...
      console.log("vuelta controller",response.data);
  ...
  async eliminarUsuario(req: Request, res: Response) {
    try {
      const response = await axios.delete(`${this.baseUrl}/usuarios/${req.params.idUsuario}`);
  ...
  async obtenerPorId(req: Request, res: Response) {
    try {
      const response = await axios.get(`${this.baseUrl}/usuarios/${req.params.idUsuario}`);
      res.status(200).json(response.data);
    } catch (error: any) {
      res.status(500).json({ 
        message: error.response?.data?.message || error.message 
   ...
   actualizarUsuario=async(req: Request, res: Response)=> {
    try {
      ...
      const response = await axios.put(`${this.baseUrl}/usuarios/${req.params.idUsuario}`, req.body);
      res.status(200).json(response.data);
    } catch (error: any) {
      res.status(500).json({ 
        message: error.response?.data?.message || error.message 
